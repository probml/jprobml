...
# https://github.com/QuantEcon/QuantEcon.jl/blob/master/src/kalman.jl
# To map the notation used in MLAPP ch. 18, note that
# - the book uses C, the code uses G for observation model
# - the book uses z, the code uses x for hidden state
# - the book uses mu_{t|t-1}, the code uses cur_x_hat
# - the book uses Sigma_t|t-1}, the code uses cur_sigma
# ie. the state of the KF object is the posterior predictive distribution
...
mutable struct Kalman
    ...
    R # observation noise
    k # num. hidden states
    n # num. observed states
    cur_x_hat # posterior predictive mean
    cur_sigma # posterior predictive covariance
end
